...
They have given us a netcat command to connect to a remote server and also a download - _dist.tar.gz_. To get started we have to first extract the contents of the downloaded compressed folder. As the process is similar in most of the challenges I won't be mentioning in every challenge, so please refer to the explanation of [Eval is Evil](#eval-is-evil) challenge writeup for the process to decompress.
...
When we enter the string as input to the netcat command, we will get the flag.
...
It somehow didn't work YET!, as it is not generating any invalid characters while decoding, it must be base64 encoded and from the challenge name and the hint, we can understand that it is being encoded multiple times.  
